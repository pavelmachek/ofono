				...
				ofono_sim_file_info_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, buf, crsm_prefix,
				...
				ofono_sim_write_cb_t cb, void *data)
{
	...
	ret = g_mot_chat_send(sd->chat, buf, crsm_prefix,
				...
				void *data)
{
	...
	if (g_mot_chat_send(sd->chat, "AT+CIMI", NULL,
					...
					void *data)
{
	...
	case OFONO_VENDOR_IFX:
		if (g_mot_chat_send(sd->chat, "AT+XPINCNT", xpincnt_prefix,
					xpincnt_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_SPEEDUP:
		if (g_mot_chat_send(sd->chat, "AT+CPNNUM", NULL,
					cpnnum_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_OPTION_HSO:
		if (g_mot_chat_send(sd->chat, "AT_OERCN?", oercn_prefix,
					oercn_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_HUAWEI:
		if (g_mot_chat_send(sd->chat, "AT^CPIN?", huawei_cpin_prefix,
					huawei_cpin_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_ICERA:
		if (g_mot_chat_send(sd->chat, "AT%PINNUM?", pinnum_prefix,
					pinnum_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_ZTE:
		if (g_mot_chat_send(sd->chat, "AT+ZPINPUK=?", zpinpuk_prefix,
					zpinpuk_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_MBM:
		if (g_mot_chat_send(sd->chat, "AT*EPIN?", epin_prefix,
					at_epin_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_SIMCOM:
		if (g_mot_chat_send(sd->chat, "AT+SPIC", simcom_spic_prefix,
					simcom_spic_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_TELIT:
		if (g_mot_chat_send(sd->chat, "AT#PCT", pct_prefix,
					at_pct_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_ALCATEL:
		if (g_mot_chat_send(sd->chat, "AT+PNNM?", pnnm_prefix,
					at_pnnm_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_QUECTEL:
		if (g_mot_chat_send(sd->chat, "AT+QPINC?", qpinc_prefix,
					at_qpinc_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_QUECTEL_SERIAL:
		if (g_mot_chat_send(sd->chat, "AT+QTRPIN", qtrpin_prefix,
					at_qtrpin_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_UBLOX:
		if (g_mot_chat_send(sd->chat, "AT+UPINCNT", upincnt_prefix,
					upincnt_cb, cbd, g_free) > 0)
			return;
		break;
	case OFONO_VENDOR_GEMALTO:
		if (g_mot_chat_send(sd->chat, "AT^SPIC", gemalto_spic_prefix,
					gemalto_spic_cb, cbd, g_free) > 0)
			return;
		break;
	default:
		if (g_mot_chat_send(sd->chat, "AT+CPINR", cpinr_prefixes,
			...
			void *data)
{
	...
	if (g_mot_chat_send(sd->chat, "AT+CPIN?", cpin_prefix,
			...
			ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	ret = g_mot_chat_send(sd->chat, buf, none_prefix,
				...
				ofono_query_facility_lock_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, buf, clck_prefix,
				...
				void *data)
{
	...
	if (g_mot_chat_send(sd->chat, "AT+CUAD", cuad_prefix,
		...
		ofono_sim_open_channel_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, cmd, ccho_prefix, at_open_channel_cb,
			cbd, g_free) > 0)
		return;
		...
		ofono_sim_close_channel_cb_t cb, void *data)
{
	...
	g_mot_chat_send(sd->chat, cmd, NULL, at_close_channel_cb, cbd, g_free);
}
				...
				ofono_sim_read_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, buf, crla_prefix,
		...
		ofono_sim_logical_access_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, cmd, cgla_prefix, logical_access_cb,
			cbd, g_free) > 0)
		return;
...
static void xcmscsc_query_cb(gboolean ok, GAtResult *result, gpointer user)
{
	...
	g_mot_chat_send(sd->chat, "AT+CLCK=?", clck_prefix,
...
static void at_xcmscsc_test_cb(gboolean ok, GAtResult *result, gpointer user)
{
	 ...
	 * acknowledgement
	 */
	g_mot_chat_send(sd->chat, "AT+XCMSRS=2", none_prefix,
					NULL, NULL, NULL);

	/* Query active card slot */
	g_mot_chat_send(sd->chat, "AT+XCMSCSC?", xcmscsc_prefix,
...
static int motorola_sim_probe(struct ofono_sim *sim, unsigned int vendor,
				void *data)
{
	GMotChat *chat = data;
	struct sim_data *sd;
			...
			ofono_sim_set_active_card_slot_cb_t cb, void *data)
{
	...
	if (g_mot_chat_send(sd->chat, cmd, none_prefix, xcmscsc_cb,
			cbd, g_free) > 0)
		return;
